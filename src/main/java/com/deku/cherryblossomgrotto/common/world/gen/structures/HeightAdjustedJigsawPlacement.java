...
public class HeightAdjustedJigsawPlacement {
    ...
    private static void addPieces(RandomState p_227211_, int p_227212_, boolean p_227213_, ChunkGenerator p_227214_, StructureTemplateManager p_227215_, LevelHeightAccessor p_227216_, RandomSource p_227217_, Registry<StructureTemplatePool> p_227218_, PoolElementStructurePiece p_227219_, List<PoolElementStructurePiece> p_227220_, VoxelShape p_227221_) {
        HeightAdjustedJigsawPlacement.Placer jigsawplacement$placer = new HeightAdjustedJigsawPlacement.Placer(p_227218_, p_227212_, p_227214_, p_227215_, p_227220_, p_227217_);
        jigsawplacement$placer.placing.addLast(new HeightAdjustedJigsawPlacement.PieceState(p_227219_, new MutableObject<>(p_227221_), 0));

        while(!jigsawplacement$placer.placing.isEmpty()) {
            HeightAdjustedJigsawPlacement.PieceState jigsawplacement$piecestate = jigsawplacement$placer.placing.removeFirst();
    ...
    static final class Placer {
        ...
        final Deque<HeightAdjustedJigsawPlacement.PieceState> placing = Queues.newArrayDeque();
        ...
        void tryPlacingChildren(PoolElementStructurePiece p_227265_, MutableObject<VoxelShape> p_227266_, int p_227267_, boolean p_227268_, LevelHeightAccessor p_227269_, RandomState p_227270_) {
            ...
            for(StructureTemplate.StructureBlockInfo structuretemplate$structureblockinfo : structurepoolelement.getShuffledJigsawBlocks(this.structureTemplateManager, blockpos, rotation, this.random)) {
                boolean placedChild = tryPlacing(p_227265_, structuretemplate$structureblockinfo, structuretemplatepool$projection, flag, mutableobject, boundingbox, i, p_227266_, p_227267_, p_227268_, p_227269_, p_227270_);

                //
                if (!placedChild) {
                    // This works to force fetch the risers!!
                    // How can I get a riser in a particular direction though? Like to make sure I dont connect to the wall in a way where I just go up even more?
                    ResourceKey<StructureTemplatePool> riserKey = ResourceKey.create(Registries.TEMPLATE_POOL, new ResourceLocation(MOD_ID, "great_wall/risers"));
                    Optional<? extends Holder<StructureTemplatePool>> optionalHolder = this.pools.getHolder(riserKey);
                    // TODO: Something in here is not actually spawning the riser (height check isnt reached so something is up)
                    boolean placedRiser = tryPlacing(p_227265_, structuretemplate$structureblockinfo, riserKey, false, structuretemplatepool$projection, flag, mutableobject, boundingbox, i, p_227266_, p_227267_, p_227268_, p_227269_, p_227270_);
                    if (placedRiser) {
                        continue label129;
                    }
                } else {
        ...
        boolean tryPlacing(PoolElementStructurePiece p_227265_, StructureTemplate.StructureBlockInfo structuretemplate$structureblockinfo, StructureTemplatePool.Projection structuretemplatepool$projection, boolean flag, MutableObject<VoxelShape> mutableobject, BoundingBox boundingbox, int i, MutableObject<VoxelShape> p_227266_, int p_227267_, boolean p_227268_, LevelHeightAccessor p_227269_, RandomState p_227270_) {
            ResourceKey<StructureTemplatePool> resourcekey = readPoolName(structuretemplate$structureblockinfo);
            return tryPlacing(p_227265_, structuretemplate$structureblockinfo, resourcekey, true, structuretemplatepool$projection, flag, mutableobject, boundingbox, i, p_227266_, p_227267_, p_227268_, p_227269_, p_227270_);
        }

        boolean tryPlacing(PoolElementStructurePiece p_227265_, StructureTemplate.StructureBlockInfo structuretemplate$structureblockinfo, ResourceKey resourcekey, boolean forcePoolMatch, StructureTemplatePool.Projection structuretemplatepool$projection, boolean flag, MutableObject<VoxelShape> mutableobject, BoundingBox boundingbox, int i, MutableObject<VoxelShape> p_227266_, int p_227267_, boolean p_227268_, LevelHeightAccessor p_227269_, RandomState p_227270_) {
            //
            ...
            if (optional.isEmpty()) {
                HeightAdjustedJigsawPlacement.LOGGER.warn("Empty or non-existent pool: {}", (Object)resourcekey.location());
            } else {
                ...
                if (holder.value().size() == 0 && !holder.is(Pools.EMPTY)) {
                    ...
                    if (holder1.value().size() == 0 && !holder1.is(Pools.EMPTY)) {
                        HeightAdjustedJigsawPlacement.LOGGER.warn("Empty or non-existent fallback pool: {}", holder1.unwrapKey().map((p_255599_) -> {
                        ...
                        for(StructurePoolElement structurepoolelement1 : list) {
                            ...
                            for(Rotation rotation1 : Rotation.getShuffled(this.random)) {
                                ...
                                for(StructureTemplate.StructureBlockInfo structuretemplate$structureblockinfo1 : list1) {
                                    boolean canAttach = JigsawBlock.canAttach(structuretemplate$structureblockinfo, structuretemplate$structureblockinfo1);
                                    if (!forcePoolMatch || canAttach) {
                                        ...
                                        //
                                        // NOTE: This seems to work to stop generation if the height difference is too much, but how can I instead of using a wall, swap to a new template pool that will take its place via the risers
                                        //  If I can figure out how to do that, then the riser should be able to handle much of the elevation gain before generation continues as normal afterwards
                                        //  Will need to consider that each failed placement here is actually multiple attempts so a single break and replace won't do. I'll need to figure out how to know what the placer is out of options entirely and react to that by swapping to risers
                                        // Utilize code under ResourceKey<StructureTemplatePool> resourcekey = readPoolName(structuretemplate$structureblockinfo); maybe to try and force-read from my risers pool instead? Needs a lot of following logic to work though...... Entire thing needs to be functionized better to meet my needs I think
                                        int height = this.chunkGenerator.getFirstFreeHeight(blockpos4.getX(), blockpos4.getZ(), Heightmap.Types.WORLD_SURFACE_WG, p_227269_, p_227270_);
                                        int maxX = boundingbox2.maxX();
                                        int maxZ = boundingbox2.maxZ();
                                        int minY = boundingbox2.minY();
                                        ...
                                        // TODO: Seems to get stuck in an infinite loop when generating the wall in some cases and noticed that this if statement never gets entered
                                        //  I think whats happening is that pieces array is never added to and placing list never gets updated so the structure thinks that it needs to keep calling the while loop on placing to see if there's more left to process
                                        if (!Shapes.joinIsNotEmpty(mutableobject1.getValue(), Shapes.create(AABB.of(boundingbox3).deflate(0.25D)), BooleanOp.ONLY_SECOND)) {
                                            ...
                                            if (p_227267_ + 1 <= this.maxDepth) {
                                                this.placing.addLast(new HeightAdjustedJigsawPlacement.PieceState(poolelementstructurepiece, mutableobject1, p_227267_ + 1));
                                            }
                                            //
                                            placedChild = true;
                                            //
