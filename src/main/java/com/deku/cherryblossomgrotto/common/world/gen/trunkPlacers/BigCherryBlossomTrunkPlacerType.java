    ...
    public static TrunkPlacerType<BigCherryBlossomTrunkPlacer> createTrunkPlacerType(Codec trunkPlacerCodec) {
        ...
        try {
            ...
            if (constructorParameters.length == 1 && constructorParameters[0] == Codec.class) {
                ...
                CherryBlossomGrotto.LOGGER.error("Big Cherry Blossom Trunk Place Type instantiating with UNEXPECTED PARAMS");
                return null;
            }
        } catch (NullPointerException exception) {
            CherryBlossomGrotto.LOGGER.error("Big Cherry Blossom Trunk Place Type NOT FOUND");
        } catch (InstantiationException exception) {
            CherryBlossomGrotto.LOGGER.error("Big Cherry Blossom Trunk Place Type is instantiating an ABSTRACT CLASS");
            return null;
        } catch (IllegalAccessException exception) {
            CherryBlossomGrotto.LOGGER.error("Big Cherry Blossom Trunk Place Type is instantiating with INCORRECT PARAMS");
        }
        catch (InvocationTargetException exception) {
            CherryBlossomGrotto.LOGGER.error("Big Cherry Blossom Trunk Place Type is instantiating with an EXCEPTION");
        }
        return null;
    }
}
